...
async def get_graph_data(
    ...
    entity_data = (
        db.query(Entity.eid, Entity.nc_id, Entity.main_word, Entity.datetime, func.max(NewsCluster.datetime))
    ...
    news_data = db.query(News.title, News.nid, News.nc_id, News.datetime, Entity.main_word, Sentiment.polarity) \
    ...
    if news_data:
        for news in news_data:
            ...
            main_word = news[4]
            polarity = news[5]
            if nid not in existed_nw:
                existed_nw.append(nid)
                news_cnt += 1
                news_attributes = {
                    'label': title,
                    'cluster' : nc_id,
                ...
                source = 'news_' + str(existed_nw.index(nid))
            news_to_entity_attr = {
                'polarity': polarity
            }
            edge_id += 1
            target = "entity_" + str(existed_et.index(main_word))
